...
individuals$year <- as.factor(substr(individuals$visitdate, 1,4))
individuals$month <- as.factor(substr(individuals$visitdate,6,7))

# Loading UVA data
load("//galen.storage.virginia.edu/ivy-hip-chru/ekd6bx/UVA_ind_weather_merged")
uva <- weather_merged
uva$Principal.Diagnosis4 <- substr(uva$dx, 2, 5)
uva$Principal.Diagnosis3 <- substr(uva$dx, 2, 4)
uva$diabetic <- ifelse(uva$dx %in% all_diabetes | uva$Principal.Diagnosis4 %in% all_diabetes | uva$Principal.Diagnosis3 %in% all_diabetes,1,0)
uva$year <- as.factor(substr(uva$visitdate, 1,4))
uva$month <- as.factor(substr(uva$visitdate, 6,7))
...
weather_merged2 <- subset(uva, dx != '0')

# Looking at temperature and how this relates to diabetes-related ED visits
uva_sub <- subset(uva, is.na(Max.T.Â.F.) == F)
uva_diabetic_proportions <- uva_sub %>% group_by(Max.T.Â.F., year) %>% summarize(mean(diabetic))

individuals_sub <- subset(individuals, is.na(Max.T.Â.F.) == F)
diabetic_proportions <- individuals_sub %>% group_by(Max.T.Â.F., year) %>% summarize(mean(diabetic))

uva_diabetic_proportions <- as.data.frame(uva_diabetic_proportions)
diabetic_proportions <- as.data.frame(diabetic_proportions)

diabetic_proportions <- rbind(diabetic_proportions, uva_diabetic_proportions)
diabetic_proportions <- subset(diabetic_proportions, Max.T.Â.F. != 'M')
diabetic_proportions <- subset(diabetic_proportions, Max.T.Â.F. != '.')
ggplot(diabetic_proportions, aes(x = Max.T.Â.F., y = `mean(diabetic)`)) + geom_point() + xlab("Temperature (F)") + ylab("Proportion of Diabetes-Related ED Visits")+
  ggtitle('Proportions of ER visits due to Diabetes as a Function of Temperature') + scale_x_discrete(breaks=seq(0,95,5))

uva$day <- weekdays(as.Date(uva$visitdate))
individuals$day <- weekdays(as.Date(individuals$visitdate))
#uva$day <- factor(uva$day,levels = c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"))
#individuals$day <- factor(individuals$day,levels = c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"))
...
individuals <- individuals %>% 
  mutate(day = fct_relevel(day, "Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"))
uva_diabetes <- subset(uva, diabetic == 1)
uva_nondiabetes <- subset(uva, diabetic == 0)
carilion_diabetes <- subset(individuals, diabetic == 1)
carilion_nondiabetes <- subset(individuals, diabetic == 0)

ggplot(uva_diabetes, aes(x = factor(day))) + geom_bar(aes(y = (..count..)/sum(..count..)),fill='red') +
  xlab("Weekday") + ylab("Proportion of Visits") +ggtitle("Diabetes-Related ED Visits at UVA Between Weekdays")
ggplot(uva_nondiabetes, aes(x = factor(day))) + geom_bar(aes(y = (..count..)/sum(..count..)),fill='blue') +
  xlab("Weekday") + ylab("Proportion of Visits") +ggtitle("Non-Diabetes-Related ED Visits at UVA Between Weekdays")
ggplot(carilion_diabetes, aes(x = factor(day))) + geom_bar(aes(y = (..count..)/sum(..count..)),fill='red') +
  xlab("Weekday") + ylab("Proportion of Visits") +ggtitle("Diabetes-Related ED Visits at Carilion Between Weekdays")
ggplot(carilion_nondiabetes, aes(x = factor(day))) + geom_bar(aes(y = (..count..)/sum(..count..)),fill='blue') +
  xlab("Weekday") + ylab("Proportion of Visits") +ggtitle("Non-Diabetes-Related ED Visits at Carilion Between Weekdays")

uva_sub <- subset(uva, is.na(Windspeed.1PM) == F)
uva_diabetic_proportions <- uva_sub %>% group_by(Windspeed.1PM) %>% summarize(mean(diabetic))

individuals_sub <- subset(individuals, is.na(Windspeed.1PM) == F)
diabetic_proportions <- individuals_sub %>% group_by(Windspeed.1PM) %>% summarize(mean(diabetic))

uva_diabetic_proportions <- as.data.frame(uva_diabetic_proportions)
diabetic_proportions <- as.data.frame(diabetic_proportions)

diabetic_proportions <- rbind(diabetic_proportions, uva_diabetic_proportions)
diabetic_proportions <- subset(diabetic_proportions, Windspeed.1PM != '.')
diabetic_proportions <- subset(diabetic_proportions, Windspeed.1PM != '')

ggplot(diabetic_proportions, aes(x = Windspeed.1PM, y = `mean(diabetic)`)) + geom_point() + xlab("Wind Speed") + ylab("Proportion of Diabetes-Related ED Visits")+
  ggtitle('Proportions of ER visits due to Diabetes as a Function of Wind Speed')+ scale_x_discrete(breaks=seq(0,10,1))

# 1AM, 7AM, 1PM, 7PM
uva_sub <- subset(uva, is.na(AT.Â.F..1AM) == F)
uva_temp <- select(uva_sub, AT.Â.F..1AM, AT.Â.F..7AM, AT.Â.F..1PM, AT.Â.F..7PM, diabetic)
carilion_sub <- subset(individuals, is.na(AT.Â.F..1AM) == F)
carilion_temp <- select(carilion_sub, AT.Â.F..1AM, AT.Â.F..7AM, AT.Â.F..1PM, AT.Â.F..7PM, diabetic)
temp <- rbind(uva_temp,carilion_temp)
#temp$av_app_temp <- rowMeans(temp[1:4],na.rm=T)
as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}
temp$AT.Â.F..1AM <- as.numeric.factor(temp$AT.Â.F..1AM)
temp$AT.Â.F..7AM <- as.numeric.factor(temp$AT.Â.F..7AM)
temp$AT.Â.F..1PM <- as.numeric.factor(temp$AT.Â.F..1PM)
temp$AT.Â.F..7PM <- as.numeric.factor(temp$AT.Â.F..7PM)
temp$diabetic <- as.factor(temp$diabetic)
temp$av_app_temp <- rowMeans(temp[1:4],na.rm=T)

temp_diabetes <- subset(temp, diabetic == 1)
temp_nondiabetes <- subset(temp, diabetic == 0)
h1 <- ggplot(temp_diabetes, aes(x=av_app_temp)) + geom_histogram(aes(y=..count../sum(..count..)),fill='red') +  ylab("Proportion")+xlab('Average Apparent Temperature') + ggtitle("Distribution of ED Visits as a Function of Average Apparent Temperature")
h2 <- ggplot(temp_nondiabetes, aes(x=av_app_temp)) + geom_histogram(aes(y=..count../sum(..count..)),fill='blue') + xlab("Average Apparent Temperature")+ ylab("Proportion")
library(gridExtra)
grid.arrange(h1, h2, ncol = 1)

ggplot(data = individuals) +
  geom_histogram(aes(x = Age, y = ..density.., fill = diabetic), position = 'dodge', binwidth = 5, alpha= 0.8) 
ggplot(data = uva) +
  geom_histogram(aes(x = age, y = ..density.., fill = diabetic), position = 'dodge', binwidth = 5, alpha= 0.8)

temp_diabetes2 <- subset(temp_diabetes,av_app_temp >= 85)
temp_nondiabetes2 <- subset(temp_nondiabetes,av_app_temp>=85)
h3 <- ggplot(temp_diabetes2, aes(x=av_app_temp)) + geom_histogram(aes(y=..count../sum(..count..)),fill='red') +  ylab("Proportion")+xlab('Average Apparent Temperature') + ggtitle("Distribution of ED Visits as a Function of Average Apparent Temperature")
h4 <- ggplot(temp_nondiabetes2, aes(x=av_app_temp)) + geom_histogram(aes(y=..count../sum(..count..)),fill='blue') + xlab("Average Apparent Temperature")+ ylab("Proportion")
library(gridExtra)
grid.arrange(h3, h4, ncol = 1)
