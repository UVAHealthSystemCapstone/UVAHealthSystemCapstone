...
prop_avg = mean(individuals$diabetic == 1)
prop_by_wkd <- individuals %>% group_by(weekdays) %>% summarise(p = mean(diabetic == 1))
ggplot() +
  ...
  ggtitle("Diabetes-Related ED Visits at Carilion Between Weekdays")


# manually
test <- individuals %>% group_by(weekdays) %>% summarize(prop = mean(diabetic == 1), n = n()) %>%
...
# logistic regression
#data = model.matrix(diabetic ~ weekdays-1, data = individuals)
intercept <- prop_avg
X <- data.frame(model.matrix(~weekdays-1,data =individuals))
dat <- X %>% mutate(response = individuals$diabetic)
fit = lm(log(response) ~ .,data = dat)
coef(glm.fit)
summary(glm.fit)
...
dia_wkd <- individuals %>% select(weekdays, diabetic) %>% filter(diabetic == 1) %>% 
  group_by(weekdays) %>% 
  summarise(n = n()) %>%
  mutate(prop = prop.table(n))
...
# mdl = lm(prop ~ weekdays, data = dia_wkd)
