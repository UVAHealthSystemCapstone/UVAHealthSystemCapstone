...
individuals$year <- substr(individuals$visitdate,1,4)
...
uva <- weather_merged
uva$year <- substr(uva$visitdate,1,4)
uva$month<- substr(uva$visitdate, 6,7)

uva$dx4 <- substr(uva$dx, 2, 5)
uva$dx3 <- substr(uva$dx, 2, 4)
uva$diabetic <- ifelse(uva$dx %in% all_diabetes | uva$dx4 %in% all_diabetes | uva$dx3 %in% all_diabetes,1,0)

# Comparison tests by hosptials
#loop through hospitals
hospitals <- c('CFMH', 'CGCH', 'CNRV', 'CRMH', 'CSJH', 'CTCH')


# Proportion of Individuals with Diabetes by Hospital
diabetic_proportions <- individuals %>% group_by(Facility, year,month) %>% summarize(mean(diabetic))

# Use this line to group all Carilion facilities into one
# diabetic_proportions$Facility <- as.factor('Carilion')

#add UVA
uva_diabetic <- uva %>% group_by(year,month) %>% summarize(mean(diabetic))
uva_diabetic$Facility <- as.factor('UVA')
...
diabetic_proportions <- subset(diabetic_proportions, year > 2009 & Facility != "")
diabetic_proportions$Facility <- factor(diabetic_proportions$Facility)


year <- ggplot(diabetic_proportions, aes(x = year, y = mean.diabetic., color = Facility, group = Facility)) +
  geom_smooth( se = FALSE) + xlab("Year") + ylab('Proportions of ER visits due to Diabetes')
month <- ggplot(diabetic_proportions, aes(x = month, y = mean.diabetic., color = Facility, group = Facility)) +
  geom_smooth( se = FALSE) + xlab("Month") + ylab('Proportions of ER visits due to Diabetes')
library(gridExtra)
grid.arrange(year,nrow=1)
grid.arrange(month,nrow=1)
