...
#load in diabetes codes:
ICD10 <- read.csv("//galen.storage.virginia.edu/ivy-hip-chru/ekd6bx/ICD10.txt", header = FALSE)

individuals$hospZip <- 0

individuals$hospZip <- ifelse(individuals$Facility == "BMH", 24523, 
                              ifelse(individuals$Facility == "CRCH", 24013,
                              ifelse(individuals$Facility == "CFMH", 24151,
                              ifelse(individuals$Facility == "CTCH", 24651,
                              ifelse(individuals$Facility == "CNRV", 24073,
                              ifelse(individuals$Facility == "CGCH", 24134,
                              ifelse(individuals$Facility == "CSJH", 24450,
                              ifelse(individuals$Facility == "CRMH", 24014, "ERROR"))))))))

#Create Factors
individuals$hospZip <- as.factor(individuals$hospZip)

individuals$Gender <- as.factor(individuals$Gender)

#Create dummies
summary(individuals$Gender)
individuals <- cbind(dummy_cols(individuals$Gender),individuals)

#Get Unique list of all zip codes of patients in the data:
patientZips <- individuals %>% group_by(Zip) %>% summarise(nPatients = n())

#Keep five digit zip codes that begin with two
patientZips <- subset(patientZips, patientZips$Zip > 19999 & patientZips$Zip < 30000)
names(patientZips) <- c('zip','nPatients')

data(zipcode)
allZips <- zipcode
allZips$zip <- as.integer(allZips$zip)

patientZips <- left_join(patientZips, allZips)

#limit to patients in VA and create columns with Hosptial zip code
patientZips <- subset(patientZips, patientZips$state == 'VA')
patientZips$BMH <- 24523
patientZips$CRCH <- 24013
patientZips$CFMH <- 24151
patientZips$CTCH <- 24651
patientZips$CNRV <- 24073
patientZips$CGCH <- 24134
patientZips$CSJH <- 24450
patientZips$CRMH <- 24014

write.csv(patientZips, "//galen.storage.virginia.edu/ivy-hip-chru/ekd6bx/pzips.csv")



library(readxl)

zdist <- read_excel('//galen.storage.virginia.edu/ivy-hip-chru/ekd6bx/pzip_data.xlsx')


VaPatientZips <- c(patientZips$zip)
...
VAindividuals <- filter(individuals, individuals$Zip %in% VaPatientZips)
VAindividuals <- rename(VAindividuals, zip = Zip)

#CFMH
CFMH <- zdist[c('zip', 'CFMH' , paste0('z','CFMH','time'), paste0('z', 'CFMH', 'dist'))]
colnames(CFMH) <- c('zip', 'hospZip', "time", 'dist')
CFMH$hospZip <- as.factor(CFMH$hospZip)


#CGCH
CGCH <- zdist[c('zip', 'CGCH' , paste0('z','CGCH','time'), paste0('z', 'CGCH', 'dist'))]
colnames(CGCH) <- c('zip', 'hospZip', "time", 'dist')
CGCH$hospZip <- as.factor(CGCH$hospZip)



#CNRV
CNRV <- zdist[c('zip', 'CNRV' , paste0('z','CNRV','time'), paste0('z', 'CNRV', 'dist'))]
colnames(CNRV) <- c('zip', 'hospZip', "time", 'dist')
CNRV$hospZip <- as.factor(CNRV$hospZip)


#CRMH
CRMH <- zdist[c('zip', 'CRMH' , paste0('z','CRMH','time'), paste0('z', 'CRMH', 'dist'))]
colnames(CRMH) <- c('zip', 'hospZip', "time", 'dist')
CRMH$hospZip <- as.factor(CRMH$hospZip)


#CSJH
CSJH <- zdist[c('zip', 'CSJH' , paste0('z','CSJH','time'), paste0('z', 'CSJH', 'dist'))]
colnames(CSJH) <- c('zip', 'hospZip', "time", 'dist')
CSJH$hospZip <- as.factor(CSJH$hospZip)


#CTCH
CTCH <- zdist[c('zip', 'CTCH' , paste0('z','CTCH','time'), paste0('z', 'CTCH', 'dist'))]
colnames(CTCH) <- c('zip', 'hospZip', "time", 'dist')
CTCH$hospZip <- as.factor(CTCH$hospZip)


VAindividuals <- left_join(VAindividuals, CFMH, by = c('hospZip', 'zip'))
VAindividuals <- left_join(VAindividuals, CGCH, by = c('hospZip', 'zip'))
VAindividuals <- left_join(VAindividuals, CNRV, by = c('hospZip', 'zip'))
VAindividuals <- left_join(VAindividuals, CRMH, by = c('hospZip', 'zip'))
VAindividuals <- left_join(VAindividuals, CSJH, by = c('hospZip', 'zip'))
VAindividuals <- left_join(VAindividuals, CTCH, by = c('hospZip', 'zip'))

VAindividuals <- replace_na(c(VAindividuals$time.x, VAindividuals$time.x.x, VAindividuals$time.x.x.x, VAindividuals$time.y, VAindividuals$time.y.y, VAindividuals$time.y.y.y), 0)

mergeVars <- c("time.x", 'time.x.x', 'time.x.x.x', 'time.y', 'time.y.y', 'time.y.y.y','dist.x', 'dist.x.x', 'dist.x.x.x', 'dist.y', 'dist.y.y', 'dist.y.y.y')
VAindividuals[mergeVars][is.na(VAindividuals[mergeVars])] <- 0

VAindividuals$dist <- VAindividuals$dist.x + VAindividuals$dist.x.x + VAindividuals$dist.x.x.x + VAindividuals$dist.y + VAindividuals$dist.y.y + VAindividuals$dist.y.y.y
VAindividuals$time <- VAindividuals$time.y + VAindividuals$time.y.y + VAindividuals$time.y.y.y + VAindividuals$time.x + VAindividuals$time.x.x + VAindividuals$time.x.x.x

VAindividuals <- subset(VAindividuals, zip != 23014)

VAindividuals <- data.frame(VAindividuals)

VAdiabetics <- subset(VAindividuals, diabetic == 1)
VANondiabetics <- subset(VAindividuals, diabetic == 0)

summary(VAdiabetics$time)
summary(VANondiabetics$time)

summary(VAdiabetics$dist)
summary(VANondiabetics$dist)
