...
uva_miss <- uva %>% filter(is.na(Max.T.Â.C.))
summary(uva_miss)
uva_res <- uva_miss %>% group_by(Station, visitdate) %>% count() %>% data.frame()
write.csv(uva_res,"uva_miss.csv")
carilion_miss <- individuals %>% filter(is.na(Max.T.Â.C.))
car_res <- carilion_miss %>% group_by(Station, visitdate) %>% count() %>% data.frame()
write.csv(car_res, "car_miss.csv")
