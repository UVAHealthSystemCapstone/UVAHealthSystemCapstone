...
# Carilion Climate Subset - can modify to include variables of interest
carilion_climate <- individuals %>% select(1, 3, 12:115, 118)
carilion_climate <- na.omit(carilion_climate)
as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}
carilion_climate$AT.Â.F..1AM <- as.numeric.factor(carilion_climate$AT.Â.F..1AM)
carilion_climate$AT.Â.F..7AM <- as.numeric.factor(carilion_climate$AT.Â.F..7AM)
carilion_climate$AT.Â.F..1PM <- as.numeric.factor(carilion_climate$AT.Â.F..1PM)
carilion_climate$AT.Â.F..7PM <- as.numeric.factor(carilion_climate$AT.Â.F..7PM)
carilion_climate$av_app_temp <- rowMeans(carilion_climate[c(28,49,70,91)],na.rm=T)

# Creating new dataframe with sum of diabetes patients at certain average apparent temperatures
carilion_climate$nondiabetic <- ifelse(carilion_climate$diabetic == 0, 1, 0)
carilion_climate2 <- carilion_climate %>% group_by(visitdate, Facility, av_app_temp) %>% summarize(d=sum(diabetic), n=sum(nondiabetic))

# Proportion of diabetes patients
carilion_climate2$prop_diabetes <- carilion_climate2$d / (carilion_climate2$d + carilion_climate2$n)

# UVA Climate Subset
uva_climate <- uva %>% select(1, 2, 10:113, 116)
uva_climate <- na.omit(uva_climate)

as.numeric.factor <- function(x) {as.numeric(levels(x))[x]}
uva_climate$AT.Â.F..1AM <- as.numeric.factor(uva_climate$AT.Â.F..1AM)
uva_climate$AT.Â.F..7AM <- as.numeric.factor(uva_climate$AT.Â.F..7AM)
uva_climate$AT.Â.F..1PM <- as.numeric.factor(uva_climate$AT.Â.F..1PM)
uva_climate$AT.Â.F..7PM <- as.numeric.factor(uva_climate$AT.Â.F..7PM)
uva_climate$av_app_temp <- rowMeans(uva_climate[c(28,49,70,91)],na.rm=T)

# Sum of diabetes and nondiabetes patients
uva_climate$nondiabetic <- ifelse(uva_climate$diabetic == 0, 1, 0)
uva_climate2 <- uva_climate %>% group_by(visitdate, Station, av_app_temp) %>% summarize(d=sum(diabetic), n=sum(nondiabetic))

# Proportion of diabetes patients
uva_climate2$prop_diabetes <- uva_climate2$d / (uva_climate2$d + uva_climate2$n)

ggplot(uva_climate2, aes(x=av_app_temp, y=prop_diabetes)) + geom_point() + 
  xlab("Average Apparent Temperature (°F)") + ylab("Proportion of Diabetes Visits")
