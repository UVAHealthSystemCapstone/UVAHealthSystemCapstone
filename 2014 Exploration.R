...
diabetic_proportions <- individuals %>% group_by(month, Facility, year) %>% summarize(mean(diabetic))
...
ggplot(diabetic_proportions, aes(x = month,y = mean.diabetic., color = Facility, group = Facility)) +
  geom_smooth(se = FALSE) + xlab("Month") + ylab('Proportions of ER visits due to Diabetes') + ylim(0.005,0.02) +
